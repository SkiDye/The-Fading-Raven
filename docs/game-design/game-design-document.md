...
|-----------|------------------|------|
| 왕국 | 성간 연합 (Stellar Union) | 배경 |
| 섬 | 우주 정거장 (Station) | 스테이지 |
| 집 | 시설 모듈 (Module) | 자원 건물 |
| 바이킹 | 해적단 + 폭풍 생명체 | 적 세력 |
| 바이킹 전선 | **Storm Line (폭풍 전선)** | 시간 제한 |
| 왕의 죽음 | 중앙 본부 함락 | 배경 스토리 |
| 탈출 | 최종 점프 게이트 도달 | 승리 조건 |

### 1.4 TFR 용어 정의

> Bad North 용어를 TFR 테마에 맞게 통일

| 원본 용어 | TFR 용어 | 설명 |
|----------|----------|------|
| Commander | **Team Leader (팀장)** | 분대 지휘관 |
| Replenish | **Resupply (재보급)** | 시설에서 크루 회복 |
| Flee | **Emergency Evac (긴급 귀환)** | Raven 셔틀로 철수 |
| Boat | **Drop Pod/Shuttle (침투정)** | 적 수송선 |
| Wave | **Storm Line 웨이브** | 적 침공 단위 |
...
- **핵심 차별점**: Raven 드론 시스템 (Scout/Flare/Resupply/Orbital Strike)
...
| 조작 | 효과 |
|------|------|
| 크루 선택 | **Tactical Mode** 진입 (시간 감속) |
| 타일 클릭 | 이동 명령 |
| 스킬 버튼 | 특수 능력 발동 |
| 시설 클릭 | **Resupply** (재보급) 명령 |
| Raven 버튼 | 드론 능력 사용 |
| 스페이스바 | 일시정지 토글 |
| Q / E | 카메라 회전 |
| 마우스 휠 | 카메라 줌 |
...
### 2.3 Tactical Mode (전술 모드)

> 기존 "슬로우 모션" → TFR "Tactical Mode"로 재설계
> Raven AI가 전술 지원을 제공하는 연출
...
**효과**
- 게임 속도 **~0.3배** 감속
- HUD에 "TACTICAL MODE" 표시
- 스캔라인 + 청색 톤 오버레이 (선택적)
- Raven AI 음성: "분석 중..." (선택적)
...
> Bad North의 "집"을 다양화 - **각 시설이 고유 보너스 제공**

| 시설 | 크레딧 | 특수 보너스 | 활성 조건 |
|------|--------|------------|----------|
| **거주 모듈** | 1~3 | 없음 (기본) | - |
| **의료 모듈** | 2 | **Resupply 시간 -50%** | 방어 성공 시 |
| **무기고** | 2 | **해당 스테이지 데미지 +20%** | 방어 성공 시 |
| **통신탑** | 1 | **Raven 드론 능력 +1회** | 방어 성공 시 |
| **발전소** | 3 | **터렛 성능 +50%** | 방어 성공 시 |

**시설 보너스 상세**

| 시설 | 보너스 효과 | 전략적 가치 |
|------|------------|------------|
| 의료 모듈 | 8명 분대: 16초 → **8초** 재보급 | 장기전, 다중 웨이브 |
| 무기고 | 모든 아군 데미지 +20% | 브루트 대응, 빠른 클리어 |
| 통신탑 | Scout/Flare/Resupply/Strike +1회 | Raven 의존 전략 |
| 발전소 | 터렛 DPS +50%, 체력 +50% | 엔지니어 빌드 |

**우선순위 가이드**
- 폭풍 스테이지: **통신탑** > 의료 (조명탄 +1)
- 브루트 웨이브: **무기고** > 통신탑 (화력 필요)
- 엔지니어 운용: **발전소** > 무기고 (터렛 강화)
- 일반 스테이지: **의료** > 거주 (지속력)
...
| 노드 | 아이콘 | 설명 |
|------|--------|------|
| **STATION** | 일반 | 기본 전투 정거장 |
| **RESCUE** | 🆘 구조 | 구조 임무 - 탈출자 합류 → 새 팀장 |
| **SALVAGE** | 🔧 잔해 | 난파선 탐색 - 전투 후 장비 획득 |
| **DEPOT** | 📦 보급 | 보급 정거장 - 무료 장비 (비전투) |
| **STORM_STATION** | ⚡ 폭풍 | 폭풍 스테이지 - 시야 제한, 조명탄 필요 |
| **ELITE_STATION** | 💀 엘리트 | 강화 적 + 고보상 |
| **BOSS** | 👑 보스 | 해적 대장 또는 Storm Core |
| **SHOP** | 🛒 상점 | 장비/업그레이드 구매 |
| **REST** | 🏥 휴식 | 무료 회복 + 선택적 업그레이드 |
| **BEACON** | 📡 비콘 | 비콘 활성화 - 진행 저장 (체크포인트) |
| **GATE** | 🚪 게이트 | 최종 점프 게이트 - 탈출 목표 |
...
**규칙**
- 탈출자는 전투 중 시설에서 대기 (Resupply 중인 것처럼)
- 탈출자가 있는 시설 파괴 시 → 영입 실패
- 영입 성공 시: **새 팀장 + 기본 크루 4명** 획득
- **긴급 귀환 사용 시 영입 실패** (탈출자 버림)
- 같은 특성 팀장 중복 등장 안 함

### 10.6 긴급 귀환 (Emergency Evac)

> 기존 "Flee (도주)" → TFR "Emergency Evac"로 재설계
> Raven 기함이 회수 셔틀을 파견하여 크루를 철수시킴

**긴급 귀환 시퀀스**
```
[Emergency Evac 버튼 클릭]
       ↓
[Raven 기함에서 회수 셔틀 하강]
       ↓
[셔틀이 에어락 위치에 착륙]
...
**귀환 조건**
- 에어락 타일이 적에게 점령되지 않아야 함
- **RESCUE 노드에서는 탈출자 버림** (영입 실패)
- **Fearless 특성 보유 팀장은 귀환 불가**

**귀환 중 메카닉**
- 레인저는 셔틀로 이동 중에도 사격 가능
- 귀환 명령 취소 불가 (셔틀 호출 후)
- 셔틀 도착까지 약 **5초** 대기
...
| 요소 | 위치 | 설명 |
|------|------|------|
| 크루 버튼 | 좌하단 | 원형, 클래스 색상 배경 |
| 크루 이름 | 버튼 아래 | 팀장 이름 표시 |
| 명령 버튼 3개 | 버튼 우측 | Move/Skill/Resupply |
| Raven 버튼 4개 | 우측 | Scout/Flare/Resupply/Strike |
| 일시정지 | 우상단 | **\|\|** 아이콘 |
| 크레딧 표시 | 우상단 | 코인 아이콘 + 숫자 |
| 웨이브 정보 | 상단 중앙 | "Wave X/Y" |
...
| 상태 | 시각 표현 |
|------|----------|
| 선택됨 | 크루 위 **녹색 원형 아이콘** (깃발 위) |
| 이동 가능 | 타일에 **흰색 핀 마커** |
| 비선택 | 기본 상태 (표시 없음) |
| Tactical Mode | HUD에 "TACTICAL MODE" 텍스트 + 청색 오버레이 |

#### 녹색 선택 아이콘 명세

- **모양**: 원형 (지름 24px)
- **색상**: #4ADE80 (녹색)
- **위치**: 선택된 크루 깃발/머리 위 32px
- **애니메이션**: 부드러운 상하 부유 (선택적)
...
|------|--------|------|--------|
| 1 | 핀 마커 📍 | **Move** (이동) | 좌클릭 |
| 2 | 스킬 아이콘 ⚡ | **Skill** (스킬 사용) | Q |
| 3 | 십자+집 🏥 | **Resupply** (재보급) | R |
...
- **화살표 개수**: 침투정 수에 비례
- **화살표 크기**: 적 총 수에 비례
- **Multiple 화살표**: 다방향 침공 시 각 방향 표시

#### Final Wave 알림

- **텍스트**: "FINAL WAVE" (화면 중앙)
- **색상**: 빨간색 + 검은 외곽선
- **지속**: 3초간 표시 후 페이드아웃
- **효과**: 배경 깜빡임 (선택적)
...
| 1 | 👁️ | **Scout** (정찰) | 전투당 2회 |
| 2 | 🔦 | **Flare** (조명탄) | 폭풍 전용 |
| 3 | 📦 | **Resupply** (긴급 보급) | 전투당 1회 |
| 4 | 💥 | **Orbital Strike** (궤도 폭격) | 전투당 1회 |
...
*문서 작성일: 2026-02-02*
*최종 수정: 2026-02-05 (UI/UX 시스템 섹션 추가)*
*The Fading Raven Game Design Document v1.3*
